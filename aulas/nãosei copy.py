...
def encontra_cliente_poremail(lista_clientes, email_busca):
    email_busca_lower = email_busca.lower()
    for cliente in lista_clientes:
        if cliente.email.lower() == email_busca_lower:
            return cliente
    return None
...
def atualizar_clientes(lista_clientes):
    ...
    email_busca = input("Digite o email do cliente que deseja atualizar: ")
    cliente_para_update = encontra_cliente_poremail(lista_clientes, email_busca)
    ...
    print(f"\nDados do cliente '{email_busca}' atualizados com sucesso!")
...
def excluir_cliente(lista_clientes):
    ...
    cliente_para_excluir = encontra_cliente_poremail(lista_clientes, email_busca)

    if cliente_para_excluir:
        lista_clientes.remove(cliente_para_excluir)
        print(f"\nCliente {cliente_para_excluir.email} excluído com sucesso!")
    else:
        print("\nCliente não encontrado.")
